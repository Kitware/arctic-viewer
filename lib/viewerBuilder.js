var // Core elements
    ...
    BinaryDataProberImageBuilder   = require('tonic-image-builder/lib/builder/BinaryDataProber'),
...
export default function build(basepath, data, config, callback) {
    ...
    if(contains(dataType, 'tonic-query-data-model')) {
        ...
        if(contains(dataType, 'in-situ-data-prober') || (contains(dataType, 'data-prober') && contains(dataType, 'binary'))) {
            ...
            viewer.imageBuilder.update();
        } else if(contains(dataType, 'webgl-composite') || contains(dataType, 'depth-composite')) {
            viewer.pipelineModel = new PipelineModel(viewer.queryDataModel.originalData);
            viewer.imageBuilder = new DepthCompositeImageBuilder(viewer.queryDataModel, viewer.pipelineModel, lutManager);
            viewer.imageBuilder.update();
            ...
            if(contains(dataType, 'multi-color-by')) {
                ...
                viewer.imageBuilder = new SortedCompositeImageBuilder(viewer.queryDataModel, lutManager);
                viewer.imageBuilder.update();
            }
        }
        callback(viewer);
