var // Core elements
    ...
    QueryDataModelWidget = require('tonic-widgets/lib/react/widget/ParameterSet/QueryDataModelWidget'),
...
export function createViewer(url, callback) {
    ...
    getJSON(url, function(error, data) {
        ...
        if(contains(dataType, 'tonic-query-data-model')) {
            ...
            if(contains(dataType, 'in-situ-data-prober') || (contains(dataType, 'data-prober') && contains(dataType, 'binary'))) {
            ...
            } else if(contains(dataType, 'volume-composite')) {
                var pipelineModel = new PipelineModel(viewer.queryDataModel.originalData),
                ...
                depthImageBuilder.setListeners(viewer.queryDataModel.getTonicMouseListener());
                colorImageBuilder.setListeners(viewer.queryDataModel.getTonicMouseListener());

                function fetchData() {
                    viewer.queryDataModel.fetchData(fetchQuery);
                }

                // Handle pipeline and data binding
                viewer.queryDataModel.onStateChange(fetchData);
                viewer.queryDataModel.on('pipeline_data', function(data, envelope) {
                    var imageSize = viewer.queryDataModel.originalData.CompositePipeline.dimensions;
                ...
                viewer.layout = '2x1';
                viewer.renderers = {
                    'Color': { builder: colorImageBuilder, name: 'Color'},
