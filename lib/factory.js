    ...
    getJSON(url, function(error, data) {
        ...
        if(contains(dataType, 'tonic-query-data-model')) {
            ...
            if(contains(dataType, 'in-situ-data-prober')) {
                var singleView = {
                        name            : 'Single View',
                        imageBuilder    : new DataProberImageBuilder(viewer.queryDataModel, lutManager),
                ...
                var dimensions = viewer.queryDataModel.originalData.InSituDataProber.dimensions,
                    imageBuilders = [
                        new DataProberImageBuilder(viewer.queryDataModel, lutManager),
                        new DataProberImageBuilder(viewer.queryDataModel, lutManager),
                        new DataProberImageBuilder(viewer.queryDataModel, lutManager)
                    ],
                    chartPainters = [
