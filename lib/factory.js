...
import ProbeViewerWidget from 'paraviewweb/src/React/Viewers/Probe3DViewer';
...
import ArcticListViewer from './viewer';
...
let buildViewerForEnsemble = false;
...
export function bindDatasets(bindConfig, renderers) {
  bindConfig.forEach((binding) => {
    ...
    const callbackFn = createQueryDataModelCallback(
      datasets,
      binding.arguments
    );
    ...
    if (binding.other) {
      binding.other.forEach((rendererBinding) => {
        const rendererCallback = createImageBuilderCallback(
          rendererList,
          rendererBinding.setter
        );
...
export function createOperators(operatorConfig, renderers) {
  operatorConfig.forEach((operator) => {
    ...
    const pixelOperator = new PixelOperatorImageBuilder(
      operator.operation,
      operator.datasets
    );
  ...
  operatorConfig.forEach((operator) => {
    const pixelOperator = renderers[operator.name].builder;
    ...
    operator.datasets.forEach((name) =>
...
export function createViewer(url, callback) {
  ...
  getJSON('/config.json', (configErr, config_) => {
    let config = Object.assign({}, config_, configOverwrite);
    if (configErr) {
      config = {};
    }
        ...
        viewerBuilder(basepath, data, config, (background) => {
          if (background.allowMagicLens) {
            viewerBuilder(basepath, data, config, (viewer) => {
              const defaultSynchList = ['phi', 'theta', 'n_pos', 'time'];
              background.imageBuilder.queryDataModel.link(
                viewer.imageBuilder.queryDataModel,
                defaultSynchList,
                true
              );
              viewer.imageBuilder.queryDataModel.link(
                background.imageBuilder.queryDataModel,
                defaultSynchList,
                true
              );

              viewer.imageBuilder = new MagicLensImageBuilder(
                viewer.imageBuilder,
...
export function createUI(viewer, container, callback) {
  if (viewer.bgColor && viewer.ui !== 'MultiViewerWidget') {
    container.style[
      viewer.bgColor.indexOf('gradient') !== -1
        ? 'background'
        : 'background-color'
    ] =
      viewer.bgColor;
  ...
  setImmediate(() => {
    const renderers = viewer.renderers || {};
    Object.keys(renderers).forEach((name) => {
  ...
  if (viewer.ui === 'ReactComponent') {
    ...
    ReactDOM.render(
      React.createElement(ReactClassMap[viewer.ui], viewer),
      container,
      callback
    );
  }
