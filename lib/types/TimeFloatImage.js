...
export default function build({ viewer, dataType }) {
  ...
  (viewer.queryDataModel.originalData.arguments_active || []).forEach(name => viewer.queryDataModel.setAnimationFlag(name, true));
  ...
  const layerName = viewer.queryDataModel.originalData.FloatImage.layers[0].name;
  floatImageBuilder.updateLayerColorBy(layerName, viewer.config.lookupTableManager.getActiveField());
  ...
  const Component = React.createClass({
    ...
    propTypes: {
      ...
      extendedProps: React.PropTypes.object,
    },
  ...
  const extendedProps = Object.assign({}, viewer.queryDataModel.originalData.initialState);
  viewer.component = (
    <Component
      ref={c => (viewer.instance = c)}
