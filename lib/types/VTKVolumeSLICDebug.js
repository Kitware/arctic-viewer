...
class ClusterSurfaceDataModel {
  constructor(queryDataModel, delta = 1.0) {
    ...
    const [
      ...
      yLength,
      zLength,
    ] = queryDataModel.originalData.SLIC.dimensions;
    const boundingBox = vtkCubeSource.newInstance({
      xLength,
      yLength,
      zLength,
      center: [xLength / 2, yLength / 2, zLength / 2],
    });
  ...
  updatePolyData(clusterBuffer) {
    ...
    for (let i = 0; i < dataSize; i += 7) {
      ...
      dataValues[offset] =
        clusterBuffer[i + 3] + this.delta * clusterBuffer[i + 5];
      ...
      pointsValues[offset * 3 + 2] = center[2];
      dataValues[offset] =
        clusterBuffer[i + 3] + this.delta * clusterBuffer[i + 4];
      this.min = Math.min(this.min, dataValues[offset]);
      this.max = Math.max(this.max, dataValues[offset]);
      offset += 1;
      center[0] -= this.delta;
      ...
      pointsValues[offset * 3 + 2] = center[2];
      dataValues[offset] =
        clusterBuffer[i + 3] + this.delta * clusterBuffer[i + 6];
      this.min = Math.min(this.min, dataValues[offset]);
      this.max = Math.max(this.max, dataValues[offset]);
      offset += 1;
