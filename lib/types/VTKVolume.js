...
export class PiecewiseWidget extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.updateVolumeBuilder = debounce(
      this.updateVolumeBuilder.bind(this),
      250
    );
...
PiecewiseWidget.propTypes = {
  volumeBuilder: PropTypes.object.isRequired,
};
...
export default function build({ basepath, viewer, dataType }) {
  ...
  const volumeBuilder = new VolumeBuilder(
    lutMgr,
    imageyDataModel,
  ...
  imageyDataModel.setDataManager(viewer.queryDataModel.getDataManager());
  imageyDataModel.setFetchGzip(
    !!viewer.queryDataModel.originalData.metadata.fetchGzip
  );
